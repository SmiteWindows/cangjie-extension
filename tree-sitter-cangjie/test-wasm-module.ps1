...
if (-not [string]::IsNullOrEmpty($WasmFilePath)) {
    ...
    $wasmDirectory = Join-Path -Path $projectRoot -ChildPath "target\$WasmTarget\release"
...
if (-not (Test-WasmFileExists -WasmFilePath $resolvedWasmFilePath)) {
    ...
    if ([string]::IsNullOrEmpty($WasmFilePath)) {
        Write-Host "📝 Tip: Build the project first using 'cargo build --target wasm32-wasip2 --release' for wasm32-wasip2 target" -ForegroundColor Yellow
        Write-Host "   or 'cargo build --target wasm32-unknown-unknown --release' for wasm32-unknown-unknown target" -ForegroundColor Yellow
    }
    exit 1
}
...
if ([string]::IsNullOrEmpty($WasmFilePath)) {
    Write-Host "   - Build for wasm32-wasip2: cargo build --target wasm32-wasip2 --release" -ForegroundColor Cyan
    Write-Host "   - Build for wasm32-unknown-unknown: cargo build --target wasm32-unknown-unknown --release" -ForegroundColor Cyan
}
Write-Host "   - Clean and rebuild: cargo clean && cargo build --target $WasmTarget --release" -ForegroundColor Cyan
